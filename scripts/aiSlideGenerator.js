...
import { MASTER_THEMES, createLayoutSlide } from './templates.js';
...
export class AiSlideGenerator {
    ...
    // 白底純文字投影片 — 委託 createLayoutSlide 工廠
    _makeWhiteSlide(gen, page) {
        const type = page.slideType || 'content';
        return createLayoutSlide(type, {
            title: page.title || '',
            content: page.content || '',
            day: page.day || '',
            speaker: page.speaker || '',
            needsVisual: page.needsVisual || false,
            _index: page._index || 0,
            number: page.number || '',
            label: page.label || '',
        ...
        for (let batch = 0; batch < designBatches; batch++) {
            ...
            try {
                ...
                const result = await ai.chat([
                    ...
                    { role: 'user', content: prompt }
                ], { model: 'claude-sonnet-4-5', temperature: 0.5, maxTokens: 8000 });
