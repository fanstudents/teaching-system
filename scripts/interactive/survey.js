    ...
    _renderThankYou(container, element, savedAnswers) {
            ...
            return `
                <a href="${c.url || '#'}" target="_blank" style="display:flex;align-items:center;gap:12px;padding:14px 16px;background:${bg}18;border:1px solid ${bg}40;border-radius:12px;margin-bottom:10px;text-decoration:none;transition:all 0.2s;" onmouseover="this.style.background='${bg}30'" onmouseout="this.style.background='${bg}18'">
                    <div style="width:40px;height:40px;border-radius:10px;background:linear-gradient(135deg,${bg},${bg}cc);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <span class="material-symbols-outlined" style="color:#fff;font-size:20px;">${c.icon || 'link'}</span>
                    </div>
                    <div>
                        <div style="font-size:14px;font-weight:600;color:#e2e8f0;">${c.title}</div>
                        <div style="font-size:12px;color:#94a3b8;">${c.desc || ''}</div>
                    </div>
                    <span class="material-symbols-outlined" style="margin-left:auto;color:${bg};font-size:18px;">arrow_forward</span>
                </a>`;
        }).join('');
        ...
        container.innerHTML = `
            <div class="survey-widget survey-done" style="max-width:480px;margin:0 auto;">
                ...
                <!-- CTA 區塊 -->
                <div style="font-size:14px;font-weight:600;color:#cbd5e1;margin-bottom:12px;text-align:center;">${sectionTitle}</div>
                ${ctaHtml}
                ...
                <div style="margin-top:20px;padding:16px;background:rgba(255,255,255,0.04);border-radius:12px;border:1px solid rgba(255,255,255,0.06);text-align:center;">
                    <div style="font-size:14px;color:#cbd5e1;line-height:1.7;">${farewell}</div>
                </div>
