                ...
                <div class="property-section">
                    ...
                    <div class="form-group">
                        ...
                        <select id="sbPreset" style="width:100%;padding:6px 10px;border:1px solid #d1d5db;border-radius:8px;font-size:13px;background:#fff;">
                            ...
                            <option value="email">📧 商業 Email 撰寫</option>
                            <option value="summary">📝 文章精準摘要</option>
                            <option value="translate">🌐 中英翻譯挑戰</option>
                            <option value="product">🛍️ 產品文案撰寫</option>
                            <option value="code_explain">💻 程式碼解說</option>
                            <option value="rewrite">✏️ 文章改寫優化</option>
                            <option value="qa">❓ 知識問答精準度</option>
                            <option value="story">📖 故事接龍創作</option>
                            <option value="debate">⚖️ 正反論述分析</option>
                            <option value="data">📊 數據圖表解讀</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">任務描述</label>
                ...
                <div class="property-section">
                    ...
                    <div class="form-group">
                        ...
                        <div style="margin-top:8px;padding:10px 12px;background:#f0f9ff;border:1px solid #bae6fd;border-radius:8px;">
                            <div style="font-size:0.78rem;font-weight:600;color:#0369a1;margin-bottom:4px;">💡 系統變數（自動代入，不需要學員手動輸入）</div>
                            <div style="font-size:0.72rem;color:#0c4a6e;line-height:1.7;">
                                <code style="background:#e0f2fe;padding:1px 4px;border-radius:3px;">{{FIELD_LIST}}</code>
                                會自動代入左邊「欄位清單」設定的欄位名稱，確保 Agent 產出的資料跟 Prompt 2 要回報的欄位一致。
                            </div>
...
- 只回傳 JSON，不加任何額外說明`;
        ...
        if (elementData.type === 'skillBattle') {
            ...
            const sbPresets = {
                ...
                product: {
                    question: `請為以下產品撰寫一段 150 字的銷售文案：\n\n產品名稱：AirPure X1 空氣清淨機\n價格：NT$12,800\n特點：\n- HEPA 13 醫療級濾網\n- 覆蓋 30 坪空間\n- 噪音低於 25 分貝\n- APP 遠端控制 + 空氣品質即時監測\n- 濾網壽命 12 個月\n\n目標客群：有小孩或過敏體質的家庭\n\n要求：文案要有吸引力，突出差異化賣點，包含行動呼籲（CTA）。`,
                    referenceAnswer: `守護家人每一口呼吸 —— AirPure X1\n\n擔心孩子的過敏問題？AirPure X1 搭載 HEPA 13 醫療級濾網，99.97% 過濾 PM2.5、花粉與塵蟎，守護 30 坪空間的純淨空氣。\n\n低於 25 分貝的超靜音設計，寶寶安睡不受打擾。透過專屬 APP 隨時掌握家中空氣品質，出門在外也能遠端啟動。濾網一年一換，維護零負擔。\n\n限時優惠 NT$12,800，給家人最好的空氣禮物。\n\n👉 立即選購，享 30 天無條件退換`,
                },
                code_explain: {
                    question: `請解釋以下 JavaScript 程式碼的功能，並指出可能的改進方向：\n\n\`\`\`javascript\nfunction debounce(fn, delay) {\n  let timer;\n  return function(...args) {\n    clearTimeout(timer);\n    timer = setTimeout(() => fn.apply(this, args), delay);\n  };\n}\n\nconst handleSearch = debounce((query) => {\n  fetch('/api/search?q=' + query)\n    .then(res => res.json())\n    .then(data => renderResults(data));\n}, 300);\n\ndocument.getElementById('search').addEventListener('input', (e) => {\n  handleSearch(e.target.value);\n});\n\`\`\`\n\n要求：\n1. 說明 debounce 的原理與用途\n2. 逐行解釋程式碼邏輯\n3. 提出至少 2 個改進建議`,
                    referenceAnswer: `## Debounce 防抖函式解說\n\n### 原理\nDebounce（防抖）是一種效能優化技巧，確保函式在最後一次觸發後等待指定時間才執行。如果在等待期間再次觸發，計時器會重置。常用於搜尋輸入、視窗 resize 等高頻事件。\n\n### 程式碼邏輯\n1. \`debounce(fn, delay)\` 接收目標函式和延遲時間\n2. 利用閉包保存 \`timer\` 變數\n3. 返回新函式，每次呼叫先清除舊計時器\n4. 設定新計時器，延遲後用 \`apply\` 保留 this 和參數執行原函式\n5. \`handleSearch\` 是防抖化的搜尋函式，300ms 延遲\n6. 監聽 input 事件，將輸入值傳給防抖函式\n\n### 改進建議\n1. 加入錯誤處理：fetch 應加 .catch() 處理網路錯誤\n2. 使用 encodeURIComponent 避免 XSS：query 直接拼接 URL 有安全風險\n3. 加入 AbortController 取消前一次未完成的請求`,
                },
                rewrite: {
                    question: `請將以下段落改寫為更專業、更有說服力的版本：\n\n「我們公司的 AI 產品很好用。它可以幫你做很多事情，比如寫文章、分析數據、還有翻譯。很多客戶都說好用。價格也不貴。如果你有興趣可以聯繫我們。」\n\n要求：\n1. 保留所有原始資訊\n2. 提升專業度和說服力\n3. 加入具體數據或案例（可合理虛構）\n4. 控制在 150 字以內`,
                    referenceAnswer: `我們的 AI 智慧助手已協助超過 500 家企業提升工作效率。整合文案生成、數據分析與多語翻譯三大核心功能，平均為團隊節省 40% 的作業時間。\n\n93% 的企業客戶在導入後首月即感受到顯著效益。方案起價每月 NT$2,990，提供 14 天免費試用。\n\n立即預約專人展示，體驗 AI 如何重新定義您的工作方式。`,
                },
                qa: {
                    question: `請回答以下知識問題，要求答案精準、有條理：\n\n「請解釋什麼是『碳中和』（Carbon Neutrality），並說明企業可以透過哪些具體策略來實現碳中和目標？請至少列出 5 種策略，並簡要說明每種策略的優缺點。」`,
                    referenceAnswer: `## 碳中和定義\n碳中和指一個組織的淨碳排放量為零——透過減少排放與碳抵消措施，使排放量與吸收量達到平衡。\n\n## 五大實現策略\n\n1. **再生能源轉換**（太陽能、風電）\n   優：長期成本低、零排放　缺：初始投資高、受天候影響\n\n2. **碳捕捉與封存（CCS）**\n   優：可處理工業排放　缺：技術成本高、儲存安全風險\n\n3. **碳權交易與碳抵消**\n   優：靈活性高、可立即實施　缺：存在「漂綠」爭議\n\n4. **供應鏈優化**（綠色採購、在地化生產）\n   優：降低 Scope 3 排放　缺：需要供應商配合、轉換期長\n\n5. **節能減碳措施**（LED、智慧建築、遠距辦公）\n   優：投資回收期短、技術成熟　缺：減碳幅度有限`,
                },
                story: {
                    question: `請根據以下開頭，續寫一個完整的短篇故事（200-300 字）：\n\n「清晨五點，林小明的手機突然發出一陣從未聽過的鈴聲。螢幕上顯示著一行字：『你有 24 小時可以改變一個決定。請選擇。』」\n\n要求：\n1. 故事要有明確的起承轉合\n2. 角色要有情感深度\n3. 結局要出人意料但合理\n4. 融入一個關於「選擇」的人生哲理`,
                    referenceAnswer: `清晨五點，林小明的手機突然發出一陣從未聽過的鈴聲。螢幕上顯示著一行字：「你有 24 小時可以改變一個決定。請選擇。」\n\n他以為是惡作劇，但腦中立刻浮現三年前的那個下午——他拒絕了前往東京的工作機會，選擇留在家鄉照顧生病的母親。\n\n他按下「確認」。瞬間，他發現自己坐在東京的辦公室裡，窗外是閃爍的霓虹。手機裡有百萬存款，社群上是光鮮的生活照。但通訊錄裡，母親的號碼旁寫著：「已故」。\n\n他崩潰地撥打那個神秘號碼：「我要改回來！」\n\n「你確定嗎？你將失去這裡的一切。」\n\n「這裡什麼都不是我的。」\n\n清晨五點，手機鈴聲再次響起。他躺在老家的床上，聽見廚房傳來母親煮粥的聲音。他微笑著，刪除了那條訊息。\n\n有些選擇看似錯過了全世界，其實是守住了整個宇宙。`,
                },
                debate: {
                    question: `請針對以下議題，分別寫出正方和反方的論述：\n\n議題：「企業應該強制要求員工每週至少進辦公室工作三天」\n\n要求：\n1. 正方和反方各提出 3 個論點\n2. 每個論點要有具體數據或案例支持\n3. 最後提供一個平衡的結論\n4. 總字數 250-350 字`,
                    referenceAnswer: `## 正方：應該強制進辦公室\n\n1. **協作效率**：微軟研究顯示，面對面溝通的創意產出比遠距高 25%。即時互動減少溝通延遲。\n2. **企業文化**：Google 調查指出，78% 的新進員工認為辦公室互動是融入團隊的關鍵。遠距容易造成疏離感。\n3. **管理透明度**：實體辦公便於績效觀察與即時回饋，降低管理盲區。\n\n## 反方：不應強制進辦公室\n\n1. **生產力提升**：史丹佛研究顯示遠距工作者生產力提高 13%，主因是減少通勤疲勞與辦公室干擾。\n2. **人才競爭力**：LinkedIn 數據顯示，提供彈性工作的職缺獲得 2.6 倍的應徵量。強制返辦公室將流失人才。\n3. **成本與環保**：員工平均每日通勤 1.5 小時，企業每人每年辦公空間成本約 NT$15 萬。\n\n## 結論\n最佳策略非二選一，而是採用混合模式：核心會議日進辦公室，其餘時間彈性安排。關鍵是建立以成果為導向的考核制度。`,
                },
                data: {
                    question: `請根據以下數據，撰寫一段分析報告：\n\n【2024 年台灣各產業 AI 導入率】\n- 金融業：72%\n- 製造業：58%\n- 零售業：45%\n- 醫療業：38%\n- 教育業：22%\n\n【AI 導入後效益（平均值）】\n- 營運成本降低：18%\n- 客戶滿意度提升：23%\n- 員工生產力提升：31%\n- 錯誤率降低：42%\n\n要求：\n1. 分析各產業差異的原因\n2. 指出最值得關注的趨勢\n3. 對導入率較低的產業提出建議\n4. 200 字以內`,
                    referenceAnswer: `## 2024 台灣產業 AI 導入分析\n\n金融業以 72% 領先，主因是數據基礎完善且監管科技（RegTech）需求驅動。製造業 58% 得益於智慧工廠浪潮。教育業僅 22%，反映預算限制與數位落差。\n\n最值得關注的趨勢：AI 導入帶來的錯誤率降低（42%）遠超預期，顯示 AI 在品質控管的價值被低估。員工生產力提升 31% 也證實 AI 是增效工具而非取代者。\n\n建議教育業與醫療業優先從「低成本高效益」場景切入，如 AI 批改作業（教育）、影像輔助判讀（醫療），以小規模試點累積信心，逐步擴大應用範圍。`,
                },
            };
